<template>
  <div class="login">
    ...
    >
      ...
      <!-- 确认密码框 -->
      <el-form-item label="确认密码" prop="checkPass2" class="shurukuang">
        <el-input type="password" v-model="ruleForm2.checkPass2"></el-input>
      </el-form-item>
      <!-- 提交按钮   重置按钮可删除 -->
      <el-form-item>
...
export default {
  ...
  data() {
    ...
    var validatePass2 = (rule, value, callback) => {
      //首先验证确认密码框是否为空如果为空则提示以下消息
      if (this.ruleForm2.checkPass2 == "") {
        callback(new Error("确认密码不能为空"));
        //验证两次密码是否一致如果正确默认为√
      } else if (this.ruleForm2.checkPass === this.ruleForm2.checkPass2) {
        callback();
      } else {
        //验证两次密码是否一致如果错误就提示以下内容
    ...
    return {
      ...
      rules2: {
        ...
        checkPass2: [{ validator: validatePass2, trigger: "blur" }],
        ...
        if (valid) {
          ...
          this.axios
            ...
            .then((res) => {
              ...
              if (res.data.code == 0) {
                this.$alert("注册提示", "用户名已经被占用", {
                  confirmButtonText: "确定",
                });
              } else {
                this.$alert("恭喜你注册成功,已登录!", {
                  confirmButtonText: "确定",
                });
                //提交Mutation
